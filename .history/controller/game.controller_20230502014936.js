...
module.exports = {
  ...
  async createNewGame(req, res) {
    try {
      ...
      //game settings
      let token_count = req.token_count;
      let player_count = req.player_count;
      const room = Math.random().toString().slice(2, 11);
      let game, gameInfo;
      if (token_count > 0 && player_count > 0) {
        game = await gameRepository.create({
          created_by: id, room: room, player_count:
            player_count, token_count: token_count
        });
      } else {
        game = await gameRepository.create({ created_by: id, room: room });
      }
      gameInfo = await gameInfoRepository.create({ game_id: game.id, user_id: id });
      for (let i = 0; i < game.token_count; i++) {
        let tokenId = 'base';
        let ludoToken = await ludoTokenRepository.create({ house_id: gameInfo.id, token_id: tokenId });
      }
      //TODO update game_info 
