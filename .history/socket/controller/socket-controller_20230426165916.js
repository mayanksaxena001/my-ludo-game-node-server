...
module.exports = class SocketController {
    ...
    selectedToken = async (data) => {
        ...
        if (data) {
            ...
            if (data.tokenId) await this.ludoGame.moveToken(data.tokenId.id, data.userId);
    ...
    diceRoll = async (data) => {
        ...
        if (data) {
            ...
            let tokensActive = await this.ludoGame.enableTokens(data);
            if (tokensActive > 0) await this.ludoGame.setDiceCastComplete(true);
            else{
                await this.ludoGame.setDiceCastComplete(false);
                await this.ludoGame.setPlayerTurn();
            }
