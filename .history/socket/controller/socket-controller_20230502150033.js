...
module.exports = class SocketController {
    ...
    diceRoll = async (data) => {
        ...
        if (data) {
            ...
            console.log('retainPos', obj["retainPos"]);
            console.log('movedToken', obj["movedToken"]);
            await this.sendUpdatedData(data);
        }
    };
    ...
    startGame = async (data) => {
        ...
        if (hasStarted && this.socket) {
            let started = { has_started: hasStarted, player_turn: this.ludoGame.getGameData().player_turn };
